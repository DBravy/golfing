"""
Residual-stream RNN probe, Dreamer-v3 style (velocity variant).
...
stream (detached) and, for each token position, predicts:
    (a) the *delta* from the layer-1 residual to the layer-2 residual at that
        same position, i.e. delta_t = r2_t - r1_t (the "velocity" that the
        transformer's second block applies to the residual stream), and
    (b) the transformer's per-token cross-entropy loss at that position.

Predicting the delta rather than the absolute target is often easier: the
probe only has to model what the next block *changes*, not re-emit the input
it already observed. The absolute target can be trivially recovered as
    r2_hat_t = r1_t + delta_hat_t.
...
Architecture per position t (along the sequence):
    ...
    heads  : feature = concat(z_t, h_t)
             delta_hat_t = decoder(feature)       # predicts r2_t - r1_t (MSE)
...
Run: python rnn_probe_velocity.py
"""
...
class ResidualRNN(nn.Module):
    ...
    At each step:
        ...
        delta_hat_t = decoder(feature_t)       # predicts r2_t - r1_t
...
def train(cfg: Config, pcfg: ProbeConfig):
    ...
    log = {
        ...
        "probe_recon": [],        # MSE of delta_hat vs (r2 - r1) (answer tokens)
    ...
    for step in range(cfg.total_steps):
        ...
        # Velocity target: what the transformer's next block *adds* to the
        # residual stream. Predicting this delta is typically easier than
        # predicting the full r_tgt, since the probe already sees r_src as
        # input and doesn't need to re-emit it.
        delta_target = r_tgt - r_src

        delta_hat, loss_hat, _ = probe(r_src)

        # Reconstruction + loss-prediction losses, averaged over answer tokens.
        answer_mask = mask  # (B, T)
        denom = answer_mask.sum().clamp(min=1.0)

        recon_per_tok = ((delta_hat - delta_target) ** 2).mean(dim=-1)  # (B, T)
        ...
        if (step + 1) % cfg.eval_every == 0 or step == 0:
            ...
            print(f"step {step+1:6d} | loss {avg_loss:.4f} | recon {avg_recon:.4f} "
                  f"| lossMSE {avg_loss_mse:.4f} | lr {lr_now:.2e} | "
...
def parse_args():
    p = argparse.ArgumentParser(description="Residual-stream RNN probe, velocity variant (Dreamer v3 style).")
    # Model
    ...
    p.add_argument("--log_dir", type=str, default="runs/rnn_probe_velocity")
...
def main():
    ...
    pcfg = ProbeConfig(
        ...
        attach=args.attach,
        w_probe_to_xform=args.w_probe_to_xform,
    )
