...
class FIFOKVCache:
    """Baseline: drop oldest tokens when over budget."""
...
class ScoringKVCache:
    ...
    def __init__(self, budget, mode="attention_recency",
        ...
        # Number of most-recent tokens that are unconditionally kept, regardless
        # of score. This protects the tail of each forward pass (question tokens,
        # generated tokens) from being evicted before they've had later queries
        # attend to them and accumulate score.
    ...
    def _evict(self):
        ...
        if K == 0:
            ...
            keep = torch.topk(scores, self.budget).indices
        else:
            # Unconditionally keep the last K positions; fill the remaining
            # (budget - K) slots by score from positions [0, L-K).
            pinned = torch.arange(L - K, L, device=device)
            eligible_scores = scores[: L - K]
            n_from_eligible = self.budget - K
            from_eligible = torch.topk(eligible_scores, n_from_eligible).indices
            keep = torch.cat([from_eligible, pinned])
...
def generate(model, tokenizer, prompt, max_new_tokens, cache, collector,
    ...
    max_pos = model.config.n_positions - 1
...
def run_one(model, tokenizer, collector, kind, budget, distance, device, verbose=False):
    ...
    if verbose:
        ...
        label = f"{kind} budget={budget} pin={pin} d={actual_d}"
